          ...
          >
            <div style={{ pointerEvents: "auto" }}>
              <Link href={`/projects/${project.slug}`} className="group block">
                {/* Horizontal split: card on one side, claim on the other.
                    Even tiles drift left → claim on right. Odd → claim on left. */}
                <article className={`flex items-center gap-10 w-[85vw] max-w-5xl ${i % 2 === 0 ? "flex-row" : "flex-row-reverse"}`}>
                  {/* Card image — ~55% width */}
                  <div className="flex-shrink-0 w-[55%]">
                    <div
                      className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-dark-700/50"
                  ...
                  <div className={`flex-1 ${i % 2 === 0 ? "text-left" : "text-right"}`}>
                    <span className="block text-xs tracking-widest uppercase text-slate-500 font-medium mb-4">
                      {project.category}
                    </span>
                    <h3 className="font-[family-name:var(--font-display)] text-3xl md:text-4xl font-semibold text-slate-100 group-hover:text-lime transition-colors duration-300 leading-tight mb-3">
                      {project.title}
                    </h3>
